...
const CustomRichTextEditor: React.FC<CustomRichTextEditorProps> = ({
  ...
  const injectCommand = (command: string, value: string | null = null) => {
  ...
  const styles = StyleSheet.create({
    ...
    buttonText: { fontWeight: 'bold', fontSize: 16, color: colors.default_dark },
    webview: {
  ...
  const FormatButton: React.FC<FormatButtonProps> = ({
    ...
    const isActive = formatKey ? activeFormats.includes(formatKey) : false;
    return (
      <TouchableOpacity
        onPress={onPress}
      ...
      >
        <Text
          style={[styles.buttonText, styleOverride, isActive && { color: 'white' }]}
        >
          {label}
        </Text>
      </TouchableOpacity>
    ...
    <html>
      <head>
        ...
        <style>
          ...
          html, body { height: 100%; padding: 5; }
        </style>
      </head>
      <body>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.toolbar}>
        ...
        <FormatButton
          label="•"
          onPress={() => injectCommand('insertUnorderedList')}
          formatKey="insertUnorderedList"
        />
        <FormatButton
          label="1.2..."
          onPress={() => injectCommand('insertOrderedList')}
          formatKey="insertOrderedList"
        />
        <FormatButton
          label="T1"
          onPress={() => injectCommand('formatBlock', 'h1')}
          formatKey="h1"
        />
        <FormatButton
          label="T2"
          onPress={() => injectCommand('formatBlock', 'h2')}
          formatKey="h2"
        />
        <FormatButton
          label="P"
          onPress={() => injectCommand('formatBlock', 'p')}
          formatKey="p"
        />
        <FormatButton
          label="☰"
          onPress={() => injectCommand('justifyLeft')}
          formatKey="justifyLeft"
          styleOverride={{ textAlign: 'left', width: 24 }}
        />
        <FormatButton
          label="☰"
          onPress={() => injectCommand('justifyCenter')}
          formatKey="justifyCenter"
          styleOverride={{ textAlign: 'center', width: 24 }}
        />
        <FormatButton
          label="☰"
          onPress={() => injectCommand('justifyRight')}
          formatKey="justifyRight"
          styleOverride={{ textAlign: 'right', width: 24 }}
        />
