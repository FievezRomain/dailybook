...
export function LinearProgress({ current, total, label, style, testID }: LinearProgressProps) {
  ...
  return <View accessible accessibilityRole="progressbar" accessibilityLabel={text} accessibilityValue={{ min: 0, max: progress.total, now: progress.current }} style={[{ width: '100%', gap: spacing.sm }, style]} testID={testID}><View style={{ height: componentTokens.navigation.linearProgressHeight, flexDirection: 'row', borderRadius: radii.xs, overflow: 'hidden', backgroundColor: colors.surfaceVariant }}>{progress.ratio > 0 ? <View style={{ flex: progress.ratio, backgroundColor: colors.primary }} /> : null}<View style={{ flex: 1 - progress.ratio }} /></View><Text style={{ color: colors.textSecondary, fontFamily: typography.fonts.medium, fontSize: typography.sizes.xs, lineHeight: 16, letterSpacing: 0.1 }}>{text}</Text></View>;
}
