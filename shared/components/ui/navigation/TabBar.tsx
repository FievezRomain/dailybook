...
export function TabBar<TId extends string>({ items, activeId, onSelect, disabled = false, fullWidthIndicator = false, style }: TabBarProps<TId>) {
  ...
  return (
    <View accessibilityRole="tablist" style={[{ height: componentTokens.navigation.tabBarHeight, flexDirection: 'row', alignItems: 'center' }, style]}>
      {items.map((item) => {
        const selected = item.id === activeId;
        const itemDisabled = disabled || item.disabled;
        return <Pressable key={item.id} accessibilityRole="tab" accessibilityLabel={item.label} accessibilityState={{ selected, disabled: itemDisabled }} disabled={itemDisabled} onPress={() => onSelect(item.id)} style={{ flex: 1, height: componentTokens.navigation.tabBarHeight, alignItems: 'center', justifyContent: 'center', borderBottomWidth: fullWidthIndicator ? (selected ? 2 : 1) : 0, borderBottomColor: fullWidthIndicator ? (selected ? colors.textPrimary : colors.border) : colors.transparent }}><Text style={{ color: itemDisabled ? colors.textDisabled : selected ? colors.textPrimary : colors.textSecondary, fontFamily: selected ? typography.fonts.semiBold : typography.fonts.medium, fontSize: selected ? typography.sizes.md : typography.sizes.control }}>{item.label}</Text>{selected && !fullWidthIndicator ? <View style={{ width: componentTokens.navigation.tabIndicator.width, height: componentTokens.navigation.tabIndicator.height, borderRadius: radii.xs, backgroundColor: itemDisabled ? colors.textDisabled : colors.textPrimary }} /> : null}</Pressable>;
      })}
    </View>
  );
}
