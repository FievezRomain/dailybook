...
export interface BannerProps { title: string; message: string; tone?: FeedbackTone; onDismiss?: () => void; blocking?: boolean; autoDismissDuration?: number; testID?: string }
export function Banner({ title, message, tone = 'info', onDismiss, blocking = false, autoDismissDuration = 4000, testID }: BannerProps) {
  const { colors } = useAppTheme();
  const visual = resolveFeedbackTone(colors, tone);
  const dismissible = Boolean(onDismiss) && !blocking;
  useEffect(() => {
    if (tone !== 'success' || !onDismiss || autoDismissDuration <= 0) return;
    const timeout = setTimeout(onDismiss, autoDismissDuration);
    return () => clearTimeout(timeout);
  }, [autoDismissDuration, onDismiss, tone]);
