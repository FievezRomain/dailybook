...
export function ActionSheet<TId extends string>({ open, title, subtitle, items, onSelect, onClose, material = 'solid', testID }: ActionSheetProps<TId>) {
  ...
  const contentHeight = Math.max(componentTokens.actionSheet.height, 118 + items.length * componentTokens.actionSheet.itemHeight);
  return <VascoBottomSheet open={open} onClose={onClose} title={title} material={material} height={contentHeight} header={header} dismissible testID={testID} contentContainerStyle={{ gap: 0, paddingHorizontal: 0, paddingTop: 0, paddingBottom: spacing.md }}><View>{items.map((item) => <ActionItem key={item.id} item={item} mode="sheet" onSelect={select} />)}</View></VascoBottomSheet>;
}
