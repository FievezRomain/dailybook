...
export function EventCard({ type, date, time, duration, title, description, animals = [], status, statusLabel, completed, completionPending = false, onCompletedChange, onPress, testID }: EventCardProps) {
  ...
  const label = `${typeLabel[type]}, ${title}, ${shownDate}${shownTime ? `, ${shownTime}` : ''}${statusLabel ? `, ${statusLabel}` : ''}`;
  const content = <><View style={{ width: 5, height: 64, borderRadius: 3, backgroundColor: visual.color }} /><View style={{ width: 58 }}><Text style={{ color: colors.textPrimary, fontFamily: typography.fonts.semiBold, fontSize: typography.sizes.md, lineHeight: 22 }}>{shownDate}</Text>{shownTime ? <DomainCaption>{shownTime}</DomainCaption> : null}</View><View style={{ flex: 1, gap: 2, opacity: completed ? 0.65 : 1 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.xs }}><Icon name={visual.icon} size="md" color={visual.color} /><Text numberOfLines={1} style={{ flex: 1, color: colors.textPrimary, fontFamily: typography.fonts.semiBold, fontSize: typography.sizes.md, lineHeight: 22 }}>Type · {typeLabel[type]}</Text></View><DomainBody numberOfLines={1}>{title}</DomainBody>{statusLabel ? <DomainCaption color={colors.error} numberOfLines={1}>{statusLabel}</DomainCaption> : description ? <DomainCaption numberOfLines={1}>{description}</DomainCaption> : null}{animals.length ? <LinkedAnimals animals={animals} /> : null}</View></>;
  ...
  return <Card onPress={onPress} accessibilityLabel={label} testID={testID} style={{ width: '100%', maxWidth: 360, minHeight: 112, flexDirection: 'row', alignItems: 'center', gap: 12, paddingLeft: 0, paddingRight: 12, paddingVertical: 12 }}>{content}</Card>;
}
