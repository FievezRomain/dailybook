...
interface OfferInformationsProps {
  withMessageFunctionality?: boolean;
}
...
const OfferInformations: React.FC<OfferInformationsProps> = ({
  withMessageFunctionality = true,
}) => {
  const { colors, fonts } = useAppTheme();

  const handleInscription = () => {
    Toast.show({ type: 'success', position: 'top', text1: 'Inscription réussie' });
  };
  ...
  const styles = StyleSheet.create({
    ...
    cell: { flex: 1, textAlign: 'center', color: colors.default_dark },
    headerCell: { color: colors.default_dark },
    table: {
      width: '100%',
      alignSelf: 'center',
      marginTop: 10,
    ...
    offerButtonPrice: { alignItems: 'center', marginTop: 10, marginBottom: 30 },
    price: { marginTop: 5 },
  ...
  return (
    <>
      {withMessageFunctionality && (
        <Text
          style={[
            ...
            styles.textFontMedium,
          ]}
        >
          Cette fonctionnalité est disponible avec la version premium
        </Text>
      )}
      <View style={styles.table}>
        ...
        {DATA.map((item) => (
          <View key={item.id} style={styles.row}>
            ...
            {item.free === 'Yes' ? (
              <AntDesign name="check-circle" size={20} style={styles.cell} color={colors.default_dark} />
            ) : (
      ...
      <View style={styles.offerButtonPrice}>
        <Button size="m" type="quaternary" onPress={() => handleInscription()}>
          <Text style={[styles.textFontMedium, { textTransform: 'uppercase' }]}>
            S'inscrire pour être averti de la sortie de la version premium
          </Text>
        </Button>
        <Text style={[styles.price, styles.textFontRegular]}>
          8.99€ /an pour les 1 000 premiers inscrits
        </Text>
      </View>
    </>
  );
