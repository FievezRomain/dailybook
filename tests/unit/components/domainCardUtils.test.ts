...
describe('domain card utilities', () => {
  it('maps every event type to its semantic color and icon', () => {
    ...
    expect(resolveEventVisual(lightColors, 'appointment')).toEqual({ color: lightColors.eventRdv, icon: 'stethoscope' });
    expect(resolveEventVisual(lightColors, 'walk')).toEqual({ color: lightColors.eventBalade, icon: 'compass' });
    ...
    expect(resolveEventVisual(lightColors, 'other')).toEqual({ color: lightColors.eventAutre, icon: 'circleCheck' });
  });
