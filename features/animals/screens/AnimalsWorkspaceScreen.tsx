...
import { ActionMenu, AnimalSelectorItem, AnimalSelectorMore, Banner, BottomBar, Button, Dialog, EmptyState, ErrorState, EventCard, FileItem, FloatingActionButton, GlobalCreateMenu, Icon, MediaUpload, MediaViewer, MetricCard, RootScreen, Skeleton, TopBar, resolveAsyncState, type GlobalCreateTarget } from '../../../shared/components/ui';
...
export function AnimalsWorkspaceScreen({ material = 'solid', onSelectTab, onNotifications = () => undefined, onAccount = () => undefined, onCreate, onAnimalAction, onAddHealth, onAddBodyPicture, onAddGalleryPicture, onOpenEvent, preferredAnimalId, feedback, onDismissFeedback }: AnimalsWorkspaceScreenProps) {
...
function AnimalInfos({ animal, onActions }: { animal: Animal; onActions: () => void }) {
  ...
  const rows = [
    ['Identification', animal.numeroidentification || 'Non renseignée'],
    ['Dates', [birth ? `Né le ${birth}` : undefined, arrival ? `Arrivé le ${arrival}` : undefined].filter(Boolean).join(' · ') || 'Non renseignées'],
    ['Origines', [animal.nompere ? `Père : ${animal.nompere}` : undefined, animal.nommere ? `Mère : ${animal.nommere}` : undefined].filter(Boolean).join(' · ') || 'Non renseignées'],
    ['Informations complémentaires', animal.informations || 'Aucune information complémentaire'],
  ];
