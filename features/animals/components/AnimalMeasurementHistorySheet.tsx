...
export function AnimalMeasurementHistorySheet({
  animal,
  type,
  readOnly = false,
  onClose,
  ...
  const state = resolveAsyncState({
    ...
    hasData: Boolean(history.data?.length),
  });
  const data = (history.data ?? [])
    ...
    .map((item) => ({
      label: new Intl.DateTimeFormat("fr-FR", {
        day: "numeric",
        month: "short",
      }).format(new Date(`${item.datemodification}T12:00:00`)),
      value: Number(item.value),
    }));
  return (
    <>
      <FormSheet
        title={`Historique · ${labels[type]}`}
        onBack={onClose}
        onClose={onClose}
        testID={`animal-${type}-history`}
      >
        {history.isError && state === "success" ? (
          <Banner
            tone="error"
            title="Mise à jour impossible"
            message="Les mesures déjà chargées restent disponibles."
            blocking
            onDismiss={undefined}
          />
        ) : null}
        {state === "loading" ? (
          ...
          <>
            <DataVisualization
              title={`${labels[type]} de ${animal.nom}`}
              type="line"
              state={data.length ? "ready" : "empty"}
              data={data}
              valueFormatter={(value) => `${value} ${units[type]}`}
              accessibilitySummary={`${data.length} mesures de ${labels[type].toLowerCase()}`}
            />
            {(history.data ?? []).map((item) => (
              <ListItem
                key={item.id}
                title={`${item.value} ${units[type]}`}
                subtitle={new Intl.DateTimeFormat("fr-FR", {
                  dateStyle: "long",
                }).format(new Date(`${item.datemodification}T12:00:00`))}
                onPress={readOnly ? undefined : () => setActionTarget(item)}
                accessibilityHint={
                  readOnly ? undefined : "Ouvre les actions de cette mesure"
                }
              />
            ))}
          </>
    ...
    {readOnly ? null : <ActionMenu
        open={Boolean(actionTarget)}
        title="Actions de la mesure"
        onClose={() => setActionTarget(undefined)}
        onSelect={(action) => {
          if (action === "edit") setEditTarget(actionTarget);
          else setDeleteTarget(actionTarget);
        }}
        items={[
    ...
    />}
      <Dialog
        ...
        description="Cette valeur sera retirée définitivement de l’historique."
        confirmLabel="Supprimer"
        loading={mutations.deleteHistory.isPending}
        onClose={() => setDeleteTarget(undefined)}
        onConfirm={async () => {
          if (!deleteTarget) return;
          await mutations.deleteHistory.mutateAsync({
            animalId: String(animal.id),
            item: type,
            historyId: String(deleteTarget.id),
          });
