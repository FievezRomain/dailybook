...
import { formatHomeDate, formatObjectiveEnd, getEventDate, getFirstName, getInitials, getLinkedAnimals, getObjectiveProgress, isHomeHeaderScrolled, splitHomeEvents, toEventCardType } from './homeUtils';
...
export function HomeScreen({ material = 'solid', onSelectTab = () => undefined, onNotifications = () => undefined, onAccount = () => undefined, onEvent = () => undefined, onObjective = () => undefined, onCreate = () => undefined }: HomeScreenProps) {
  ...
  return <><RootScreen header={header} bottomBar={bottomBar} floatingAction={<FloatingActionButton accessibilityLabel="Créer" onPress={() => setCreateOpen(true)} material={material} />} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />} onScrollOffsetChange={(offsetY) => setScrolled((current) => { const next = isHomeHeaderScrolled(offsetY); return current === next ? current : next; })} contentContainerStyle={{ gap: spacing.md, paddingBottom: spacing.xxl }} testID="home-overview">
...
function HomeContent({ events, objectives, animals, hasError, onRetry, onEvent, onObjective, onSelectTab }: { events: ReturnType<typeof splitHomeEvents>; objectives: readonly Objectif[]; animals: readonly Animal[]; hasError: boolean; onRetry: () => void; onEvent: (id: number) => void; onObjective: (id: number) => void; onCreate: (target: GlobalCreateTarget) => void; onSelectTab: (tab: MainTabId) => void }) {
  const { colors } = useAppTheme();
  return <View style={{ gap: spacing.md }}>{hasError ? <Banner tone="error" title="Mise à jour impossible" message="Vérifiez votre connexion. Les dernières données disponibles restent affichées." onDismiss={undefined} blocking /> : null}<Section title="Aujourd’hui" action={events.today.length ? 'Voir l’agenda' : undefined} onAction={events.today.length ? () => onSelectTab('agenda') : undefined}>{events.today.length ? events.today.slice(0, 1).map((event) => <EventCard key={event.id} type={toEventCardType(event.eventtype)} time={getEventDate(event).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })} title={event.nom || event.eventtype} description={event.commentaire} animals={getLinkedAnimals(event.animaux, animals)} onPress={() => onEvent(event.id)} />) : <SectionEmptyMessage title="Rien de prévu aujourd’hui" message="Profitez de la journée avec vos animaux." />}</Section><Section title="Prochains jours">{events.upcoming.length ? events.upcoming.slice(0, 1).map((event) => <EventCard key={event.id} type={toEventCardType(event.eventtype)} time={getEventDate(event).toLocaleDateString('fr-FR', { weekday: 'short' })} duration={getEventDate(event).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })} title={event.nom || event.eventtype} description={event.commentaire} animals={getLinkedAnimals(event.animaux, animals)} onPress={() => onEvent(event.id)} />) : <SectionEmptyMessage title="Aucun événement à venir" message="Les prochains événements apparaîtront ici." />}</Section><Section title="Objectifs en cours" action={objectives.length ? 'Voir tout' : undefined} onAction={objectives.length ? () => onSelectTab('tracking') : undefined}>{objectives.length ? objectives.slice(0, 1).map((objective) => { const progress = getObjectiveProgress(objective); return <ObjectiveCard key={objective.id} title={objective.title} status="active" statusLabel={`${Math.round(progress * objective.sousetapes.length)} sur ${objective.sousetapes.length} réalisées`} progress={progress} endLabel={formatObjectiveEnd(objective.datefin)} animals={getLinkedAnimals(objective.animaux, animals)} onPress={() => onObjective(objective.id)} />; }) : <SectionEmptyMessage title="Aucun objectif en cours" message="Créez un objectif lorsque vous êtes prêt." />}</Section>{hasError && !events.today.length && !events.upcoming.length && !objectives.length ? <EmptyState type="offline" onAction={onRetry} /> : null}</View>;
}
