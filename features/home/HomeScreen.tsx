...
import { formatHomeDate, formatObjectivePeriod, getDailyTaskProgress, getEventDate, getFirstName, getInitials, getLinkedAnimals, getObjectiveProgress, isHomeHeaderScrolled, splitHomeEvents, toEventCardType } from './homeUtils';
...
function HomeContent({ events, objectives, animals, hasError, onRetry, onEvent, onObjective, onSelectTab }: { events: ReturnType<typeof splitHomeEvents>; objectives: readonly Objectif[]; animals: readonly Animal[]; hasError: boolean; onRetry: () => void; onEvent: (id: number) => void; onObjective: (id: number) => void; onCreate: (target: GlobalCreateTarget) => void; onSelectTab: (tab: MainTabId) => void }) {
  ...
  const eventCard = (event: (typeof events.today)[number], date: string) => <EventCard key={event.id} type={toEventCardType(event.eventtype)} date={date} time={event.heuredebutevent} title={event.nom || event.eventtype} description={event.commentaire} animals={getLinkedAnimals(event.animaux, animals)} onPress={() => onEvent(event.id)} />;
  return <View style={{ gap: spacing.md }}>
    ...
    <Section title="Aujourd’hui" action="Voir tout" onAction={() => onSelectTab('agenda')}>
      ...
      {events.today.length ? eventCard(events.today[0], getEventDate(events.today[0]).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })) : <SectionEmptyMessage title="Rien de prévu aujourd’hui" message="Profitez de la journée avec vos animaux." />}
    </Section>
    <Section title="Prochains jours" action="Voir tout" onAction={() => onSelectTab('agenda')}>{events.upcoming.length ? eventCard(events.upcoming[0], getEventDate(events.upcoming[0]).toLocaleDateString('fr-FR', { weekday: 'short' })) : <SectionEmptyMessage title="Aucun événement à venir" message="Les prochains événements apparaîtront ici." />}</Section>
    <Section title="Objectifs en cours" action="Voir tout" onAction={() => onSelectTab('tracking')}>{activeObjectives.length ? activeObjectives.slice(0, 1).map((objective) => { const progress = getObjectiveProgress(objective); return <ObjectiveCard key={objective.id} title={objective.title} status="active" statusLabel={`${Math.round(progress * objective.sousetapes.length)} sur ${objective.sousetapes.length} réalisées`} progress={progress} endLabel={formatObjectivePeriod(objective.datedebut, objective.datefin)} animals={getLinkedAnimals(objective.animaux, animals)} onPress={() => onObjective(objective.id)} />; }) : <SectionEmptyMessage title="Aucun objectif en cours" message="Créez un objectif lorsque vous êtes prêt." />}</Section>
    {hasError && !events.today.length && !events.upcoming.length && !objectives.length ? <EmptyState type="offline" onAction={onRetry} /> : null}
  </View>;
}

function DailyTasksProgress({ done, total, progress }: { done: number; total: number; progress: number }) { const { colors } = useAppTheme(); return <View accessibilityRole="progressbar" accessibilityValue={{ min: 0, max: total, now: done, text: `${done} sur ${total} terminées` }} style={{ gap: spacing.sm }}><View style={{ height: 4, borderRadius: 2, backgroundColor: colors.surfaceVariant, overflow: 'hidden' }}><View style={{ width: `${Math.round(progress * 100)}%`, height: 4, backgroundColor: colors.primary }} /></View><Text style={{ color: colors.textSecondary, fontFamily: typography.fonts.medium, fontSize: typography.sizes.xs }}>{done} tâche{done > 1 ? 's' : ''} sur {total} réalisée{done > 1 ? 's' : ''}</Text></View>; }
