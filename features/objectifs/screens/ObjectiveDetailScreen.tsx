...
export function ObjectiveDetailScreen({ objective, animals, initialActionsOpen = false, onBack, onEdit, onDeleted, onDuplicated }: { objective: Objectif; animals: readonly Animal[]; initialActionsOpen?: boolean; onBack: () => void; onEdit: () => void; onDeleted: () => void; onDuplicated: (id?: number) => void }) {
  ...
  const toggleStep = async (stepId: number) => { await mutations.update.mutateAsync({ id: String(objective.id), body: toggleObjectiveStepPayload(objective, stepId) }); };
  return <>
    <DetailScreen header={<TopBar title="Objectif" context="detail" onBack={onBack} />} testID="objective-detail">
      <View style={{ gap: spacing.lg }}>
        ...
        {objective.sousetapes.map((step) => { const done = isObjectiveStepDone(step.state); return <Pressable key={step.id} accessibilityRole="checkbox" accessibilityState={{ checked: done, disabled: mutations.update.isPending }} accessibilityLabel={step.etape} disabled={mutations.update.isPending} onPress={() => void toggleStep(step.id)} style={{ minHeight: 58, flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: 14, backgroundColor: colors.surface }}><Text style={{ color: done ? colors.success : colors.primaryDark, fontFamily: typography.fonts.bold, fontSize: typography.sizes.lg }}>{done ? '✓' : '○'}</Text><View><Text style={{ color: colors.textPrimary, fontFamily: typography.fonts.semiBold }}>{step.etape}</Text><Text style={{ color: colors.textSecondary, fontFamily: typography.fonts.medium, fontSize: typography.sizes.xs }}>{done ? 'Terminé' : 'À faire'}</Text></View></Pressable>; })}
        {linked.length ? <LinkedAnimals animals={linked} /> : null}
      </View>
    </DetailScreen>
    <ActionSheet open={actionsOpen} title="Actions sur l’objectif" onClose={() => setActionsOpen(false)} onSelect={async (action) => { if (action === 'edit') onEdit(); else if (action === 'update') setActionsOpen(false); else if (action === 'duplicate') { const copy = await mutations.create.mutateAsync(duplicateObjectivePayload(objective)); onDuplicated(copy?.id); } else setDeleteOpen(true); }} items={[{ id: 'edit', label: 'Modifier', description: 'Mettre à jour toutes les informations', icon: 'edit' }, { id: 'update', label: 'Mettre à jour', description: 'Touchez directement une étape', icon: 'tracking' }, { id: 'duplicate', label: 'Dupliquer', description: 'Créer une copie modifiable', icon: 'more' }, { id: 'delete', label: 'Supprimer', description: 'Une confirmation sera demandée', icon: 'warning', tone: 'destructive' }]} />
