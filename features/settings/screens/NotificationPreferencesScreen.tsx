...
import { Banner, BottomBar, RootScreen, Switch, TopBar } from '../../../shared/components/ui';
...
export function NotificationPreferencesScreen({ activeTab, onSelectTab, onBack }: Props) {
  ...
  return <RootScreen header={<TopBar title="Notifications" context="detail" onBack={onBack} />} bottomBar={<BottomBar items={tabs} activeId={activeTab} onSelect={onSelectTab} />} contentContainerStyle={{ gap: spacing.lg }} testID="notification-preferences">
    ...
    <SettingsRow title="Rappel journalier" subtitle="Pour ne pas oublier d’enregistrer un événement" trailing={<Switch value={value} onValueChange={change} disabled={mutation.isPending} accessibilityLabel="Rappel journalier" testID="daily-reminder-switch" />} />
