...
export function SettingsOverviewScreen({ activeTab, onSelectTab, onBack, onOpen }: Props) {
  ...
  return <>
    <RootScreen header={<TopBar title="Compte et réglages" context="detail" onBack={onBack} />} bottomBar={<BottomBar items={tabs} activeId={activeTab} onSelect={onSelectTab} testID="main-tabs" />} contentContainerStyle={{ gap: spacing.lg, paddingTop: spacing.lg }} padded={false} testID="settings-overview">
      ...
      <View>
        ...
        <ListItem title="Apparence" subtitle="Système, clair ou sombre" leading={leading('visibilityOn')} onPress={() => onOpen('appearance')} />
        <ListItem title="Notifications" subtitle="Rappels et activité des groupes" leading={leading('notifications')} onPress={() => onOpen('notifications')} />
        <ListItem title="Sécurité" subtitle="Mot de passe et connexion" leading={leading('lock')} onPress={() => onOpen('security')} />
        <ListItem title="Données et confidentialité" subtitle="Suppression du compte" leading={leading('file')} onPress={() => onOpen('privacy')} />
        <ListItem title="Aide" subtitle="Support et informations" leading={leading('info')} onPress={() => onOpen('help')} />
        <ListItem title="Se déconnecter" leading={leading('lock')} onPress={() => setLogoutOpen(true)} />
      </View>
    </RootScreen>
    <Dialog open={logoutOpen} title="Se déconnecter ?" description="Vous devrez vous identifier à nouveau pour accéder à Vasco." confirmLabel="Se déconnecter" onClose={() => setLogoutOpen(false)} onConfirm={() => signOut()} />
  </>;
}
