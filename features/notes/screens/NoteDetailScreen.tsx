...
import { MarkdownNoteText } from "../components/MarkdownNoteText";
...
export function NoteDetailScreen({
  ...
  if (notesQuery.isLoading)
    return (
      <DetailScreen
        header={<TopBar title="Note" context="detail" onBack={onBack} />}
      >
  ...
  if (notesQuery.isError)
    return (
      ...
      >
        <ErrorState
          message="Impossible de charger cette note."
  ...
  const actions: readonly OverlayActionItem<NoteAction>[] = [
    {
      ...
      description: note.is_pinned
        ? "Retirer des notes prioritaires"
        : "Garder en haut de la liste",
      icon: "pin",
    },
    {
      id: "edit",
      label: "Modifier",
      description: "Éditer le titre et le contenu",
      icon: "edit",
    },
    {
      id: "share",
      label: "Partager",
      description: "Envoyer avec les applications du téléphone",
      icon: "share",
    },
    {
      id: "delete",
      label: "Supprimer",
      description: "Une confirmation sera demandée",
      icon: "delete",
      tone: "destructive",
    },
  ];
    ...
    <>
      ...
      >
        ...
        >
          ...
          <IconButton
            icon="moreHorizontal"
            accessibilityLabel="Actions sur la note"
            variant="ghost"
        ...
        {shareError ? (
          <Banner
            ...
            message="Le menu de partage n’a pas pu être ouvert. Réessayez."
        ...
        {deleteError ? (
          <Banner
            ...
            message="La note est conservée. Réessayez."
        ...
        {pinError ? (
          <Banner
            ...
            message="La note conserve son état actuel. Réessayez."
            onDismiss={() => setPinError(false)}
          />
        ) : null}
        {metadata ? (
          <Text
        ...
        <Card
          ...
          style={{ minHeight: 330, justifyContent: "flex-start" }}
        >
          <MarkdownNoteText value={note.note} />
        </Card>
      </DetailScreen>
      <ActionSheet
        open={actionsOpen}
        title="Actions sur la note"
        subtitle={note.titre}
        items={actions}
      ...
      <Dialog
        ...
        onConfirm={async () => {
          await mutations.remove.mutateAsync(String(note.id));
          onDeleted();
        }}
        testID="note-delete-confirmation"
      />
    </>
  );
}
