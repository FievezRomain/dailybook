...
export function formatAgendaDay(date: string, full = true) { const label = new Intl.DateTimeFormat('fr-FR', full ? { weekday: 'long', day: 'numeric', month: 'long' } : { day: 'numeric', month: 'long' }).format(new Date(`${date}T12:00:00`)); return label.charAt(0).toLocaleUpperCase('fr-FR') + label.slice(1); }
