...
export function EventCreateSuccessScreen({ onAgenda, onCreateAnother }: EventCreateSuccessScreenProps) {
  const form = useEventWizardStore((state) => state.formData); const animals = useAnimalsQuery().data ?? []; const selectedAnimals = animals.filter((animal) => (form.animaux ?? []).includes(animal.id)).map((animal) => ({ id: String(animal.id), name: animal.nom, imageUrl: animal.image })); const reminder = Boolean(form.notif);
