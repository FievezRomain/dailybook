...
import ItemAnimalPicker from "../../inputs/ItemAnimalPicker";

const ModalAddAnimal = ({ isVisible, setVisible, group = {}, onModify = undefined }) => {
    ...
    const changeSelectedAnimals = (animal) => {
        ...
        if(found){
            ...
            if (foundSelected){
                setValue("animals", selected.filter(e => e.id !== animal.id).map(e => e.id));
                setSelected(selected.filter((a) => a.id !== animal.id));
            } else{
                if(selected.length === 0){
                    setValue("animals", animaux.filter(e => e.id === animal.id).map(e => e.id));
                    setSelected(animaux.filter((a) => a.id === animal.id));
                } else{
                    setValue("animals", selected.concat(animaux.filter(e => e.id === animal.id)).map(e => e.id));
                ...
                <View style={styles.formContentContainer}>
                    <FlatList
                        ...
                        renderItem={({ item }) => {
                            ...
                            return(
                                <TouchableOpacity style={{flex: 1}} onPress={()=>changeSelectedAnimals(item)} key={item.id} onTouchStart={(e) => e.stopPropagation()}>
                                    <ItemAnimalPicker
