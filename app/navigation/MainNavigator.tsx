...
import {
  ...
  ObjectiveFormSheetScreen,
  TrackingScreen,
} from "../../features/objectifs";
...
export function MainNavigator() {
  ...
  const [route, setRoute] = useState<
    ...
    | { name: "event"; eventId: number; feedback?: string }
    | { name: "objective"; objectiveId: number; actionsOpen?: boolean }
    | { name: "objectiveForm"; mode: "create" | "edit"; objectiveId?: number }
    | { name: "notes" }
    | { name: "note"; noteId: number }
    ...
    | { name: "wishForm"; mode: "create" | "edit"; wishId?: number }
    | { name: "contacts" }
    | { name: "contact"; contactId: number }
    | { name: "contactForm"; mode: "create" | "edit"; contactId?: number }
    | { name: "groups" }
  ...
  const openSettings = () => {
    setNotificationsOpen(false);
    setSettingsOpen("overview");
  };
  const openNotificationTarget = (notification: Notification) => {
    setNotificationsOpen(false);
    if (!notification.object_id) return;
    if (notification.type === "event")
      setRoute({ name: "event", eventId: notification.object_id });
    else if (notification.type === "objective")
      setRoute({ name: "objective", objectiveId: notification.object_id });
    else if (notification.type === "note")
      setRoute({ name: "note", noteId: notification.object_id });
    else if (notification.type === "wish")
      setRoute({ name: "wish", wishId: notification.object_id });
  ...
  const create = (target: string, initialEventDate?: string) => {
    ...
    if (target === "objective") {
      resetObjectiveWizard();
      setRoute({ name: "objectiveForm", mode: "create" });
      return;
    }
    if (target === "wish") {
      setRoute({ name: "wishForm", mode: "create" });
      return;
    }
    if (target === "contact") {
      setRoute({ name: "contactForm", mode: "create" });
      return;
    }
    if (target === "group") {
      setRoute(
  ...
  const tracking = () => (
    <TrackingScreen
      ...
      onOpenObjective={(objectiveId) =>
        setRoute({ name: "objective", objectiveId })
      }
  ...
  const plus = () => (
    <PlusHubScreen
      ...
      onContacts={() => setRoute({ name: "contacts" })}
      onNotes={() => setRoute({ name: "notes" })}
      onWishes={() => setRoute({ name: "wishes" })}
      onCreate={create}
      onNotifications={openNotifications}
      onAccount={openSettings}
    />
  ...
  const openAnimalAction = (
    action: SensitiveAnimalAction | "edit",
    animalId: number,
  ) => {
    const animal = animals.find((item) => item.id === animalId);
    if (!animal || isSharedAnimal(animal)) return;
    setRoute(
      action === "edit"
        ? { name: "animalForm", mode: "edit", animalId }
        : { name: "animalAction", action, animalId },
    );
  };
  const rootContext = () => (
    <View style={{ flex: 1 }}>
      <View style={{ flex: 1, display: tab === "home" ? "flex" : "none" }} accessibilityElementsHidden={tab !== "home"} importantForAccessibility={tab === "home" ? "auto" : "no-hide-descendants"}>
        <HomeScreen
          onSelectTab={selectTab}
          onEvent={(eventId) => setRoute({ name: "event", eventId })}
          onObjective={(objectiveId) => setRoute({ name: "objective", objectiveId })}
          onCreate={create}
          onNotifications={openNotifications}
          onAccount={openSettings}
        />
      </View>
      <View style={{ flex: 1, display: tab === "tracking" ? "flex" : "none" }} accessibilityElementsHidden={tab !== "tracking"} importantForAccessibility={tab === "tracking" ? "auto" : "no-hide-descendants"}>
        {tracking()}
      </View>
      <View style={{ flex: 1, display: tab === "agenda" ? "flex" : "none" }} accessibilityElementsHidden={tab !== "agenda"} importantForAccessibility={tab === "agenda" ? "auto" : "no-hide-descendants"}>
        <AgendaScreen
        onSelectTab={selectTab}
      ...
      <View style={{ flex: 1, display: tab === "animals" ? "flex" : "none" }} accessibilityElementsHidden={tab !== "animals"} importantForAccessibility={tab === "animals" ? "auto" : "no-hide-descendants"}>
        <AnimalsWorkspaceScreen
        onSelectTab={selectTab}
        preferredAnimalId={preferredAnimalId}
        feedback={animalFeedback}
        onDismissFeedback={() => setAnimalFeedback(undefined)}
        onCreate={create}
        onOpenEvent={(eventId) => setRoute({ name: "event", eventId })}
      ...
      <View style={{ flex: 1, display: tab === "more" ? "flex" : "none" }} accessibilityElementsHidden={tab !== "more"} importantForAccessibility={tab === "more" ? "auto" : "no-hide-descendants"}>
        {plus()}
      </View>
    </View>
  );
  const eventContext = (eventId: number) => (
    <EventDetailScreen
      ...
      onEdit={(id) => setRoute({ name: "eventEditDetails", eventId: id })}
      onDuplicate={(sourceEventId) =>
        setRoute({ name: "eventDuplicateDetails", sourceEventId })
      }
    />
  );
  if (settingsOpen === "notifications")
    return (
      <NotificationPreferencesScreen
        activeTab={tab}
        onSelectTab={selectTab}
        onBack={() => setSettingsOpen("overview")}
      />
    );
  ...
  if (settingsOpen === "security")
    return (
      <SettingsSecurityScreen
        activeTab={tab}
        onSelectTab={selectTab}
        onBack={() => setSettingsOpen("overview")}
        onChangePassword={() => setSettingsOpen("changePassword")}
      />
    );
  if (settingsOpen === "changePassword")
    return (
      <SettingsChangePasswordScreen
        activeTab={tab}
        onSelectTab={selectTab}
        onBack={() => setSettingsOpen("security")}
      />
    );
  if (settingsOpen === "privacy")
    return (
      <SettingsPrivacyScreen
        activeTab={tab}
        onSelectTab={selectTab}
  ...
  if (settingsOpen === "overview")
    return (
      <SettingsOverviewScreen
        activeTab={tab}
        onSelectTab={selectTab}
        onBack={() => setSettingsOpen(undefined)}
  ...
  if (route.name === "groupForm") {
    const selectedGroup = route.groupId
      ? groupItems.find((item) => item.id === route.groupId)
      : undefined;
    const background = selectedGroup ? (
      <GroupDetailScreen
        groupId={selectedGroup.id}
        onBack={() => setRoute({ name: "groups" })}
        onEdit={() => undefined}
        onDeleted={() => setRoute({ name: "groups" })}
  ...
  if (route.name === "groupInvitation")
    return (
      <GroupInvitationScreen
        invitationId={route.invitationId}
        onBack={() => setRoute({ name: "groups" })}
        onAccepted={() => setRoute({ name: "groups" })}
        onDeclined={() => setRoute({ name: "groups" })}
      />
    );
  if (route.name === "group")
    return (
      <GroupDetailScreen
        ...
        onEdit={() =>
          setRoute(
            premium
              ? { name: "groupForm", mode: "edit", groupId: route.groupId }
              : { name: "groupPremium" },
          )
        }
        onDeleted={() => setRoute({ name: "groups" })}
  ...
  if (route.name === "noteVoicePremium")
    return (
      <PremiumRequiredPattern
        feature="voiceNotes"
        onBack={() => setRoute({ name: "noteCreateChoice" })}
        onComparePlans={() => setVoicePlans(true)}
        testID="voice-premium-required"
      />
    );
  if (route.name === "noteVoiceRecording")
    return (
      <VoiceRecordingScreen
        onCancel={() => setRoute({ name: "root" })}
        onRecorded={(recording) => {
          setVoiceRecording(recording);
          setRoute({ name: "noteVoiceProcessing" });
        }}
      />
    );
  if (route.name === "noteVoiceProcessing")
    return voiceRecording ? (
      <VoiceProcessingScreen
        ...
        onTranscribed={(draft) => {
          setVoiceDraft(draft);
          setRoute({ name: "noteVoiceReview" });
        }}
      />
    ) : (
      notes()
    );
  if (route.name === "noteVoiceReview")
    return voiceRecording && voiceDraft ? (
      <VoiceReviewScreen
        recording={voiceRecording}
        draft={voiceDraft}
        onCancel={() => {
  ...
  if (route.name === "noteForm") {
    const selectedNote = route.noteId
      ? noteItems.find((item) => item.id === route.noteId)
      : undefined;
    return (
      <>
        {route.noteId ? (
  ...
  if (route.name === "wishForm") {
    const selectedWish = route.wishId
      ? wishItems.find((item) => item.id === route.wishId)
      : undefined;
    const background = selectedWish ? (
      <WishDetailScreen
        wishId={selectedWish.id}
        activeTab={tab}
        onBack={() => setRoute({ name: "wishes" })}
        onEdit={() => undefined}
  ...
  if (route.name === "wish")
    return (
      <WishDetailScreen
        ...
        onBack={() => setRoute({ name: "wishes" })}
        onEdit={() =>
          setRoute({ name: "wishForm", mode: "edit", wishId: route.wishId })
        }
        onDeleted={() => setRoute({ name: "wishes" })}
        onSelectTab={selectTab}
      />
  ...
  if (route.name === "contactForm") {
    const selectedContact = route.contactId
      ? contactItems.find((item) => item.id === route.contactId)
      : undefined;
    const background = selectedContact ? (
      <ContactDetailScreen
        contactId={selectedContact.id}
        activeTab={tab}
        onBack={() => setRoute({ name: "contacts" })}
        onEdit={() => undefined}
  ...
  if (route.name === "objectiveForm") {
    const objective = route.objectiveId
      ? objectives.find((item) => item.id === route.objectiveId)
      : undefined;
  ...
  if (route.name === "animalForm") {
    const animal = route.animalId
      ? animals.find((item) => item.id === route.animalId)
      : undefined;
    if (route.mode === "edit" && (!animal || isSharedAnimal(animal)))
      return rootContext();
    return (
      <>
        {rootContext()}
            ...
            onSaved={(savedAnimal) => {
              ...
              setAnimalFeedback(
                route.mode === "edit"
                  ...
                  : `${savedAnimal.nom} a bien été ajouté à vos animaux.`,
              );
              resetAnimalWizard();
              setTab("animals");
              setRoute({ name: "root" });
  ...
  if (route.name === "animalAction") {
    ...
    if (!animal || isSharedAnimal(animal)) return rootContext();
    return (
      <>
        {rootContext()}
        <AnimalActionDialogs
      ...
      <>
        ...
        <FormSheetHost>
          <EventCreateOptionsScreen
            ...
            onBack={() =>
              setRoute(
                eventFormData.updateScope === "series" ||
                  eventFormData.updateScope === "following"
                  ? { name: "eventEditAnimals", eventId: route.eventId }
                  : { name: "eventEditDetails", eventId: route.eventId },
              )
            }
  ...
  if (route.name === "eventEditAnimals")
    return (
      <>
        {eventContext(route.eventId)}
        <FormSheetHost>
          <EventCreateAnimalsScreen
            onBack={() =>
              setRoute({ name: "eventEditDetails", eventId: route.eventId })
            }
            onClose={() => setRoute({ name: "event", eventId: route.eventId })}
            onContinue={() =>
  ...
  if (route.name === "eventEditDetails")
    return (
      <>
        {eventContext(route.eventId)}
        <FormSheetHost>
          <EventCreateDetailsScreen
            ...
            onContinue={() =>
              setRoute(
                eventFormData.updateScope === "occurrence"
                  ? { name: "eventEditOptions", eventId: route.eventId }
                  : { name: "eventEditAnimals", eventId: route.eventId },
              )
            }
          />
  ...
  if (route.name === "eventDuplicateAnimals")
    return (
      <>
        {eventContext(route.sourceEventId)}
        <FormSheetHost>
  ...
  if (route.name === "eventDuplicateDetails")
    return (
      <>
        {eventContext(route.sourceEventId)}
        <FormSheetHost>
  ...
  if (route.name === "eventCreateAiPremium")
    return (
      <PremiumRequiredPattern
        feature="aiCreation"
          ...
          <EventCreateEntryScreen
            ...
            onAi={() => {
              if (isPremiumSubscription(user?.subscription))
                setRoute({ name: "eventCreateAiDescription" });
              else setRoute({ name: "eventCreateAiPremium" });
            }}
          />
        </FormSheetHost>
      </>
  ...
  if (route.name === "event")
    return (
      <EventDetailScreen
        eventId={route.eventId}
        initialFeedback={route.feedback}
        onBack={() => setRoute({ name: "root" })}
        onDeleted={() => setRoute({ name: "root" })}
        onEdit={(eventId) => setRoute({ name: "eventEditDetails", eventId })}
        onDuplicate={(sourceEventId) =>
          setRoute({ name: "eventDuplicateDetails", sourceEventId })
        }
      />
  ...
  if (route.name === "objective") {
    ...
    return (
      <ObjectiveDetailScreen
        objective={objective}
        animals={animals}
        initialActionsOpen={route.actionsOpen}
        onBack={() => setRoute({ name: "root" })}
        onEdit={() =>
          setRoute({
            name: "objectiveForm",
  ...
  if (route.name === "note")
    return (
      <NoteDetailScreen
        ...
        onBack={() => setRoute({ name: "root" })}
        onEdit={() =>
          setRoute({ name: "noteForm", mode: "edit", noteId: route.noteId })
        }
        onDeleted={() => setRoute({ name: "root" })}
      />
    );
